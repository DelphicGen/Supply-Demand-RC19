  ...
  return (
    <div className={`flex ${props.landingPage ? 'flex-row' : 'flex-col'} ${props.divClassName}`}>
      ...
      <select
        ...
        className={`${props.landingPage ? 'w-auto mb-2' : 'w-full mb-3'} bg-gray-400 bg-gray-400 text-gray-700 p-2 rounded-md w-20 text-center font-semibold text-sm capitalize outline-none focus:shadow-outline`}
        style={{ width: props.width, maxWidth: props.maxWidth, height: 40 }}
        onChange={handleChange}>
        if(props.arrayList) {
          props.arrayList.map((item, index) => (
            <option key={props.landingPage ? item.name : item.id} value={item.name} className="text-gray-700 font-semibold text-xs outline-none border-none capitalize" >
              {item.name}
            </option>
          ))
        }
